...
# # from yolomehmet import boxess
# # label = ['person']
# # font = cv2.FONT_HERSHEY_PLAIN
# # final_images =[]

# # # boxes = boxess[2]
# # # confidences = [predicted[2]]
# # # indexes = cv2.dnn.NMSBoxes(boxes, confidences, 0.5, 0.4)
# # for j in range(len(boxess)):
# #     boxes = boxess[j]
# #     confidences = [predicted[j]]
# #     indexes = cv2.dnn.NMSBoxes(boxes, confidences, 0.5, 0.4)
...
# #         label = str(label)
# #         color = (0, 255, 0)
# #         cv2.rectangle(test_images[j], (x, y), (x + w, y + h), color, 1)
# #         (cv2.putText(test_images[j], label, (x, y + 30), font, 1.5, color, 2))

# #     cv2.imwrite('img{}'.format(j)+'.jpg',test_images[j]) 

# #     cv2.imshow("Image ", test_images)

# # # ##### convert images to video back #########
# # # import glob
